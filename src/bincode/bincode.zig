...
const arrayListInfo = utils.types.arrayListInfo;
...
pub fn testRoundTrip(deserialized_item: anytype, bincode_serialized_bytes: []const u8) !void {
    ...
    const actual_bytes = try bincode.writeAlloc(allocator, deserialized_item, .{});
    defer allocator.free(actual_bytes);
    try std.testing.expectEqualSlices(u8, bincode_serialized_bytes, actual_bytes);

    const actual_struct = try bincode.readFromSlice(allocator, T, actual_bytes, .{});
    defer actual_struct.deinit(allocator);
    try std.testing.expect(utils.types.eql(deserialized_item, actual_struct));
}
...
test "default value" {
    const FooBar = struct {
        ...
        pub const @"!bincode-config:a": bincode.FieldConfig([]u8) = .{
            .skip = true,
            .default_value = &.{},
        };
    };
