...
const base58 = @import("../base58/root.zig");
...
const ClusterType = @import("types.zig").ClusterType;
...
pub const Client = struct {
    ...
    pub fn sendTransaction(
        ...
        const written = try bincode.writeToSlice(&buffer, transaction, .{});

        const sized = crypto.base58.Base58Sized(PACKET_DATA_SIZE);
...
test "getBalance" {
    // if (true) return error.SkipZigTest;
    const allocator = std.testing.allocator;
    var client = Client.init(allocator, .Localnet, .{});
    defer client.deinit();
    const pubkey = try Pubkey.fromString("Bkd9xbHF7JgwXmEib6uU3y582WaPWWiasPxzMesiBwWm");
    const response = try client.getBalance(allocator, pubkey, .{});
    defer response.deinit();
    // _ = try response.result();
    const so = try response.result();
    std.debug.print("Balnace: {d}", .{so.value});
}
...
test "getVoteAccounts response parses correctly" {
    ...
    try std.testing.expect(utils.types.eql(expected, actual));
}
